...
const userLogin = (req, res) => {
    ...
    db.query(sql, [req.body.email], (err, data) => {
        if (err) return res.json({ message: "Server is down" });
...
const addFood = async(req, res) => {
    ...
    try {
        const queryFood = "INSERT INTO food (makanan, gambar, type, harga) VALUES (?, ?, ?, ?)"
        const [result] = await connection.execute(queryFood, [nama, imagePath, type, harga])
...
const deleteFood = async (req, res) => {
    ...
    try {
        ...
        res.status(201).json({message: "Data berhasil dihapus", data: deleteResult})
    } catch (error) {
        console.error("🔥 Error saat menghapus data:", error.message);
        res.status(500).json({ message: "Gagal meghapus data", error: error.message });
    }
}
...
const createOrder = async (req, res) => {
    ...
    try {
        ...
        const queryOrder = "INSERT INTO payment_history (customer, totalprice, create_at) VALUES (?, ?, ?)";
        const [orderResult] = await connection.execute(queryOrder, [nama, total, date]);
        ...
        const queryItems = "INSERT INTO order_items (payment_id, makanan, quantity) VALUES ?";
...
const updateFood = async (req, res) => {
  ...
  const getTopFoods = async (req, res) => {
    ...
    try {
      ...
      res.status(200).json({ message: "Berhasil ambil top 10 makanan", data: rows });
    } catch (error) {
      console.error("🔥 Error saat ambil top makanan:", error.message);
      res.status(500).json({ message: "Gagal ambil top makanan", error: error.message });
    }
  };
  ...
  const getTotalIncome = async (req, res) => {
    try {
      const connection = db.promise();
      const [rows] = await connection.execute("SELECT SUM(totalprice) AS total FROM payment_history");
      
      res.json({ total: rows[0].total || 0 });
    } catch (error) {
      console.error("🔥 Error saat mengambil total income:", error.message);
      res.status(500).json({ message: "Gagal mengambil total income", error: error.message });
    }
  };
  ...
  const getSalesByFoodType = async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({ message: "Gagal ambil data penjualan", error: error.message });
    }
  };
...
export {getFood, getOneFood, userLogin, verifyToken, addFood, deleteFood, createOrder, getHistory, updateFood, getTopFoods, getBuyer, getTotalIncome, getSalesByFoodType}